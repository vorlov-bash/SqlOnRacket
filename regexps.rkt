...
(define (main-regex query)
  (query-to-hash (remove-false-from-list (regexp-match #px"(select) (?:(distinct) (.*)|(MED)\\((\\w*)\\)|(COUNT)\\((\\w*)\\)|(SUM)\\((\\w*)\\)|(.*)) (from) (\\w*)(?:\\s)?(?:(where)\\(([\\w\\d\\s=,]*)\\)|(orderby)\\(([\\w\\d=,]*(?:\\sASC|\\sDESC)?)\\))?(?:\\s)?(?:(orderby)\\(([\\w\\d=,]*(?:\\sASC|\\sDESC)?)\\))?"
                                                       query)) (make-hash)))
